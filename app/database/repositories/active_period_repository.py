...
class ActivePeriodRepository:
    ...
    async def create(self, user: User, transaction: Transaction, start_date: datetime = datetime.now(UTC).replace(tzinfo=None), end_date: datetime = datetime.now(UTC).replace(tzinfo=None)) -> None:
